...
export function useGame(transport: GameTransport = createLocalTransport()) {
  ...
  // Le rangement des dés dans les huit cadres vit à part : il ne dépend ni du
  // transport, ni du minuteur, ni de la partie (cf. `useDiceSlots`).
  const { slots, slotOfDie, moveToSlot, syncSlots, clearSlots } = useDiceSlots(MAX_DICE)
